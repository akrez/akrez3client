...
<?php if ($this->context->id == 'site' && in_array($this->context->action->id, ['index'])): ?>
    <div class="row pb20">
        <div class="col-sm-12">
            <?php
            $logo = Html::img(Api::BlogLogo(), ['style' => 'margin: auto;', 'class' => 'img-responsive img-rounded']);
            echo Html::a($logo, Api::BlogFirstPageUrl(), ['style' => 'text-align: center;']);
            ?>
        </div>
    </div>
<?php endif; ?>
...
<?php if ($this->context->id == 'site' && $this->context->action->id == 'category'): ?>
    ...
    <?php
    echo Html::beginForm(Api::url('site', 'category', ['id' => $dp['categoryId']]), 'GET');
    ?>
    ...
    <div class="row">
        ...
        <?php
        $i = 0;
        foreach ($dp['search'] as $fieldId => $filters) :

            $field = $dp['fields'][$fieldId];
            $type = $field['type'];
            $typeFilter = $field['filter'];
            $typeOperations = (isset($dp['opertaions'][$typeFilter]) ? $dp['opertaions'][$typeFilter] : []);
            ...
            foreach ($filters as $filter) :
                ...
                <?php elseif ($typeFilter == FieldList::FILTER_2STATE) : ?>
                    ...
                    <div class="col-sm-12 pb20">
                        ...
                        <?= Html::checkbox($namePrefix . '[value]', strval($filter['value']), $dp['categories'], ['separator' => ' ']); ?>
                        <?= Html::tag('label', $field['title'], ['class' => 'control-label', 'style' => 'margin-top: 3px;margin-bottom: 2px;']) ?>
                    </div>
    ...
    <div class="row pb20">
        <div class="col-xs-12">
            <div class="btn-group-vertical" role="group" style="width: 100%;">
                <?php
                foreach (Api::getDpAttribute('categories') as $id => $title) {
                    $url = Api::url('site', 'category', ['id' => $id]);
